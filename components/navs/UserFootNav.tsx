...
export const UserFootNav = () => {
    const router = useRouter();
    const pathName = usePathname();

    return (
        <ToggleGroup className={`sticky bottom-0 h-fit w-full ${(pathName.startsWith("/message/") ? "hidden":"flex")} items-center justify-between bg-background rounded-none border-t-1 border-border px-4 py-1 md:hidden`} type="single" spacing={4} value={pathName}>
